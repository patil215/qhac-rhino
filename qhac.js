    ...
    Districts.austin = {
        ...
        api: {
            ...
            disambiguate: {
                ...
                method: 'POST',
                isRequired: function (dom) {
                    return dom.find('#_ctl0_ddlStudents').length != 0;
                },
                makeQuery: function (id, state) {
                    return {
...
(function (GradeRetriever) {
    ...
    */
    function login(district, uname, pass, studentID, success, fail) {
        // get the login page
        ...
        function do_login(doc) {
            ...
            // perform login
            new XHR('POST', district.api.login.url).success(disambiguate).fail(fail).params(query).send();
        }

        // select a student
        function disambiguate(doc) {
            // load the page DOM
            var $dom = document.createElement('div');
            $dom.innerHTML = doc;
